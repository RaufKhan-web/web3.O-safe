...
const ExecutedTransactions = () => {
    ...
    useEffect(() => {
        const fetchAllPendingTransactions = async () => {
            try {
                const response = await axios.post("http://localhost:3000/getAllApprovedTransactions", { contractAddress });
        ...
        <div className="flex flex-col gap-10">
            <div className={`${isError ? "opacity-50 pointer-events-none" : ""}`}>
                <div className="bg-white p-6 rounded-lg shadow-md">
                    <PrevNavigate />
                    <h2 className="text-2xl font-bold mb-4 text-teal-800 pl-12">Executed Trasactions</h2>
                    <p>See the details of executed transactions</p>
                </div>
                <div className="flex flex-col justify-center py-12 h-auto w-full items-center">
                    {transactions.length > 0 ? (
                        ...
                        <div>No Approved transactions was found</div>
                    )}
                </div>
            </div>
            {isError && (
